...
(DEFRULE GRAMMAR (AND SPACING (+ DEFINITION) SPACING ENDOFFILE)
  (:DESTRUCTURE
   ...
   `(PROGN (DEFPACKAGE :PEG-GRAMMAR (:USE :CL :ESRAP :CL-HEREDOC)) (IN-PACKAGE :PEG-GRAMMAR) ,@DEF)))
...
(DEFRULE PSEQUENCE (* PREFIX)
  (:DESTRUCTURE
   (&REST PREF)
   (IF PREF (IF (AND (CONSP PREF) (> (LENGTH PREF) 1)) `(AND ,@PREF) (FIRST PREF)) (VALUES))))
...
(DEFRULE ESCCHAR (AND "\\" (OR "n" "r" "t" "'" "\"" "[" "]" "\\"))
  (:DESTRUCTURE
   (SL C)
   (DECLARE (IGNORE SL))
   (CASE (CHAR C 0) (#\n #\Newline) (#\r #\Return) (#\t #\Tab) (OTHERWISE (CHAR C 0)))))
(DEFRULE NUMCHAR1
    (AND "\\" (CHARACTER-RANGES (#\0 #\2)) (CHARACTER-RANGES (#\0 #\7)) (CHARACTER-RANGES (#\0 #\7)))
  (:DESTRUCTURE
   (SL N1 N2 N3)
   (DECLARE (IGNORE SL))
...
(DEFRULE COMMENT (AND "#" (* (AND (! ENDOFLINE) CHAR1)) (OR ENDOFLINE ENDOFFILE))
